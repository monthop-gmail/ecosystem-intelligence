...
## เทียบ provider ด้วยชุดคำถามทดสอบ

```bash
make eval P=offline,claude,chatgpt SAVE=1
```

```text
  provider     model                    ผ่าน     เวลา      token in/out     cache
  ───────────────────────────────────────────────────────────────────────────────
  offline      rule-engine              4/5        0.31s  —                —
```

รายงานบอกด้วยว่า provider แต่ละตัว **หาเจอเพิ่ม** หรือ **หลุดไป** อะไรเทียบกับ baseline
— ตัวเลข pass/fail อย่างเดียวไม่บอกว่า LLM เพิ่มคุณค่าตรงไหน

### baseline ตก 1 ข้อโดยธรรมชาติ

`unanswerable-budget` ถามเรื่องงบประมาณและ headcount ซึ่ง ecosystem graph ไม่มีข้อมูลเลย
คำตอบที่ถูกคือ `answerable: false` — rule engine ทำไม่ได้เพราะมันเดินตามกฎที่เขียนไว้
ไม่ได้เข้าใจคำถาม

**นี่คือช่องว่างที่ LLM ควรปิด และเป็นเหตุผลที่ต้องมีเคสนี้ในชุดทดสอบ** —
ถ้า LLM ก็ตกข้อนี้ด้วย แปลว่า prompt ยังไม่ได้สื่อว่าการตอบว่า "ตอบไม่ได้" เป็นคำตอบที่ยอมรับได้
